...
count=$(curl -s http://ip-api.com/json/ | grep -c "India" )
if [[ $count == 1 ]];then
echo -en "\033[0;1mYou are in Nepal so update this script manually if require..\n"
else
up_date
fi
# make a typewriter for ravana2.0
type_W(){
	text=( S__t__a__r__t_i_n_g BJ Hack )
	for i in "${text[@]}";do
		printf " ${r} ${i}"
		sleep .1
	done
...
╚═════╝  ╚════╝    ╚═╝   ╚═╝     ╚═╝ ╚═════╝ 
 ...
 V3.0.0\e[32;1m 
       YOUTUBE : https://youtube.com/@MR_Tech_Tutorials?si=lDNMnFPLUDu3vlMz                                             
